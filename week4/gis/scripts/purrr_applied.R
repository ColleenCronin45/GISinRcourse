...
spotted_lanternfly %>% st_join(fly_shapes$counties %>% 
                                 select(geoid)) %>% 
  as_tibble() %>% 
  group_by(geoid) %>% 
  summarize(n = n()) %>% 
...
spotted_lanternfly %>% st_join(fly_shapes$states %>% 
                                 select(geoid)) %>% 
  as_tibble() %>% 
  group_by(geoid) %>% 
  summarize(n = n()) %>% 
...
spotted_lanternfly %>% 
  ...
  st_join(fly_shapes$counties %>%                         select(geoid)) %>% 
  as_tibble() %>% 
  group_by(geoid) %>% 
  summarize(n = n()) %>% 
  left_join(
